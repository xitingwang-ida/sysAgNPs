...
sys_ggradar <- function(plot.data,
                    base.size = 15,
                    font.radar = "sans",
  ...
  # only the first row is taken into account in the function sys_CalculateGroupPath.
  plot.data <- aggregate(
    x = plot.data[, -1],
    by = list(plot.data[, 1]),
    FUN = "mean")
  ...
  group <- NULL
  group$path <- sys_CalculateGroupPath(plot.data.offset)

  # print(group$path)
  # (c) Calculate coordinates required to plot radial variable axes
  axis <- NULL
  axis$path <- sys_CalculateAxisPath(var.names, grid.min + abs(centre.y), grid.max + abs(centre.y))
  # print(axis$path)
  ...
  gridline <- NULL
  gridline$min$path <- sys_funcCircleCoords(c(0, 0), grid.min + abs(centre.y), npoints = 360)
  gridline$mid$path <- sys_funcCircleCoords(c(0, 0), grid.mid + abs(centre.y), npoints = 360)
  gridline$max$path <- sys_funcCircleCoords(c(0, 0), grid.max + abs(centre.y), npoints = 360)
  ...
  if (!is.null(group.colours)) {
    ...
    colour_values <- sys_generate_color_values(length(unique(plot.data[, 1])))
  }
